...
- Nimbus JOSE+JWT (`com.nimbusds:nimbus-jose-jwt`, declared directly at compile scope and **shaded into the plugin jar** by `maven-shade-plugin`), used by `OAuthResourceServerAuthenticator` for JWT verification. It used to arrive transitively through the `fess` dependency, which pulled `oauth2-oidc-sdk` until the SSO authenticators moved out to the `fess-sso-*` plugins. The war supplies none of it now, and a jar in `WEB-INF/plugin` gets no dependency resolution, so it has to travel inside this jar. This is not only an `mcp.auth.mode=oauth` concern: `McpApiManager` initialises an `OAuthResourceServerAuthenticator` field eagerly, so without the shade `container.init()` fails with `NoClassDefFoundError` under every auth mode, including the default `none`
